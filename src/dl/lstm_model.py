...
File containing LSTM Aggregated/Binned Hawkes Process estimation (eta/mu)
...
class LSTM(nn.Module):
    ...
    # Spread inputs through layers and returns outputs

    def forward(self, x: torch.Tensor) -> torch.Tensor:

        """
        Performs forward pass through LSTM layers and returns output tensor

        Args:
            x (torch.Tensor): Input shape tensor (batch_size, seq_length, input_size)
